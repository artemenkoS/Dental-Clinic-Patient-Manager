...
export const getDoctors = async (req: Request, res: Response) => {
  try {
    ...
    const result = await db.query(
      `SELECT id, name, surname FROM users WHERE role = $3 LIMIT $1 OFFSET $2`,
      [pageSize, offset, doctorRole.rows[0].id]
    );
    ...
    res
      .status(500)
      .json({ message: "Произошла ошибка при получении списка докторов" });
  }
};
...
export const getOneDoctor = async (req: Request, res: Response) => {
  try {
    ...
    const user = await db.query(
      `SELECT id, name, surname, role FROM users WHERE id = $1`,
      [id]
    );
    ...
    const doctorRole = await db.query(
      `SELECT id FROM role WHERE role = 'doctor'`
    );

    if (doctorRole.rows[0].id !== user.rows[0].role) {
      return res
        .status(404)
        .json({ message: "Пользователь не является доктором." });
    }
    ...
    res
      .status(500)
      .json({ message: "Произошла ошибка при получении информации о докторе" });
  }
};
