...
export const createProcedure = async (req: Request, res: Response) => {
  ...
  try {
    const newProcedure = await db.query(
      `INSERT INTO procedure (procedire) values ($1) RETURNING *`,
      [procedure]
    );
...
export const getProcedures = async (req: Request, res: Response) => {
  try {
    ...
    const result = await db.query(
      `SELECT * FROM procedure LIMIT $1 OFFSET $2`,
      [pageSize, offset]
    );
    ...
    res
      .status(500)
      .json({ message: "Произошла ошибка при получении списка процедур" });
  }
};
...
export const getOneProcedure = async (req: Request, res: Response) => {
  try {
    ...
    const procedure = await db.query(`SELECT * FROM procedure where id = $1`, [
      id,
    ]);
    if (!procedure.rows[0]) {
...
export const updateProcedure = async (req: Request, res: Response) => {
  ...
  try {
    const newProcedure = await db.query(
      `UPDATE procedure set procedure =  $1 where id = $2 RETURNING *`,
      [procedure, id]
    );
...
export const deleteProcedure = async (req: Request, res: Response) => {
  try {
    const id = req.params.id;
    const procedure = await db.query(`DELETE FROM procedure where id = $1`, [
      id,
    ]);
    ...
    res
      .status(500)
      .json({ message: "Произошла ошибка при удалении процедуры." });
  }
};
