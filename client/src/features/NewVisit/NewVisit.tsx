...
import { useCreateVisitMutation, useGetVisitsQuery } from '../../api/visit/visitApi';
  ...
  const onSubmit = (data: FormValues) => {
    if (user && data.patient) {
      mutate({
        ...
        author: user.user.id,
        visitDate: `${format(data.visitDate, 'yyyy-MM-dd')} ${selectedTimeSlot}`,
      ...
      <Modal keepMounted open={open} onClose={handleClose}>
        <StyledBox>
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Button type="submit" variant="outlined" fullWidth disabled={!isValid}>
              Создать запись
            </Button>
          </form>
        </StyledBox>
