...
import { Control, Controller } from 'react-hook-form'; // Импорт Controller
...
interface Props {
  ...
  disabled?: boolean;
  initialPayments?: Omit<Payment, 'id'>[];
  control: Control<VisitFormValues, unknown>;
}

export const PaymentForm: React.FC<Props> = ({ formSubmit, disabled, initialPayments, control }) => {
  const [payments, setPayments] = useState<Omit<Payment, 'id'>[]>(initialPayments || []);
  ...
  const onSubmit = () => {
    console.log('PaymentForm submitted with data:', payments);
    formSubmit(payments);
    setPayments([]);
  };
